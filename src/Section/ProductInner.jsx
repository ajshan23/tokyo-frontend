...
const ProductInner = ({ product }) => {
  ...
  const handleSubmit = async () => {
    ...
    await axios
      .post(`${import.meta.env.VITE_BACKEND}/api/v1/users/createcart`, {
        productId: product._id,
        quantity: quantity,
      })
      .catch((err) => console.log(err));
    await axios
      .post(`${import.meta.env.VITE_BACKEND}/api/v1/users/loadcart`)
  ...
  const handleLike = async () => {
    if (liked) {
      await axios
        .post(
          `${import.meta.env.VITE_BACKEND}/api/v1/users/removefromwhishlist/${product._id}`
        )
        .then((response) => {
      ...
      await axios
        .post(
          `${import.meta.env.VITE_BACKEND}/api/v1/users/addtowhishlist/${product._id}`
        )
        .then((response) => {
          if (response.data.success === true) {
            setLiked(true);
  ...
  const likedOrNot = async () => {
    {
      await axios
        .post(
          `${import.meta.env.VITE_BACKEND}/api/v1/users/checkinwhishlist/${product._id}`
        )
        .then((response) => {
          if (response.data.success === true) {
            setLiked(true);
