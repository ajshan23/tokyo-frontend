...
const ListSelectedProducts = (props) => {
  ...
  const getLatest = async () => {
    await axios
      .post(`${import.meta.env.VITE_BACKEND}/api/v1/users/getlatestproduct`)
      .then((res) => setProducts(res.data.Latest));
  };
  const getFandom = async () => {
    await axios
      .post(`${import.meta.env.VITE_BACKEND}/api/v1/users/getfandom`)
      .then((res) => setProducts(res.data.fandom));
  };
  const getFeatured = async () => {
    await axios
      .post(`${import.meta.env.VITE_BACKEND}/api/v1/users/getfeatured`)
